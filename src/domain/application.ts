...
export type ApplicationSource = 'LinkedIn' | 'Referral' | 'CareerPage' | 'Other';
 ...
 */
export const VALID_STATUS_TRANSITIONS: Readonly<Record<ApplicationStatus, readonly ApplicationStatus[]>> = {
    [ApplicationStatus.APPLIED]: [ApplicationStatus.CALLBACK, ApplicationStatus.REJECTED],

    [ApplicationStatus.CALLBACK]: [ApplicationStatus.INTERVIEW, ApplicationStatus.REJECTED],
...
export function getGhostingPhase(app: JobApplication, now: Date = new Date()): GhostingPhase | null {
    ...
    if (app.status === ApplicationStatus.CALLBACK && app.firstCallbackDate && !app.firstInterviewDate) {
    ...
    if (app.status === ApplicationStatus.INTERVIEW && app.firstInterviewDate && !app.offerDate && !app.rejectionDate) {
 ...
 */
export function isPreResponseGhosted(app: JobApplication, now: Date = new Date()): boolean {

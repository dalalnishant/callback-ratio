...
export function deriveMetrics(applications: JobApplication[], now: Date = new Date()) {
    ...
    const rejections = applications.filter((a) => a.status === ApplicationStatus.REJECTED).length;
    ...
    const ghosted = preResponseGhosted + postCallbackGhosted + postInterviewGhosted;
    ...
    const avgResponseTimeDays =
        responseTimes.length === 0 ? 0 : responseTimes.reduce((a, b) => a + b, 0) / responseTimes.length;
